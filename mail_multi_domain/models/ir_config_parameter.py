        ...
        if key == "mail.catchall.domain":
            ...
            if user_config == "False":
                if (
                    ...
                    and "force_alias_domain" in self.env[self.env.context["active_model"]]._fields.keys()
                ):
                    alias_domain = (
                ...
                if not alias_domain and self._context.get("uid"):
                    alias_domain = (
                        self.env["res.users"].browse(self._context.get("uid")).company_id.sudo().force_alias_domain
                    )
